...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\textit{Emergent Quantum Gravity -- \DTMnow\ -- Version 24b}}
\fancyhead[R]{\thepage\ of \pageref{LastPage}}

% ======= Title Page =======
\title{Emergent Quantum Gravity and Dark Sectors from Spinfoam Compression}
...
EQG recovers General Relativity in the low-density limit yet predicts sharp, distinctive corrections: enhanced attraction in compressed clumps (mimicking dark matter halos), isotropic monochromatic gamma-ray lines from hidden SU(3) glueballs (10--50 GeV), scale-dependent red tilt in primordial gravitational waves (millihertz band), low-$\ell$ CMB suppression, rotation-curve enhancements at large radii, and mild phantom dark energy evolution.
...
\subsection{The EQG Model Defined}
\label{subsec:eqg_model_defined}
...
EQG predicts a set of sharp, independent, multi-messenger signatures distinct from $\Lambda$CDM and canonical LQG:
- 10--20\% suppression of CMB power at low multipoles ($\ell \lesssim 20$) from condensate fluctuations,
- scale-dependent red tilt in primordial gravitational waves, strongest at millihertz frequencies (LISA/BBO),
- isotropic monochromatic gamma-ray lines (10--50 GeV) from hidden SU(3) glueball annihilation,
- Yukawa-like corrections to galaxy rotation curves at large radii,
- mild phantom dark energy evolution ($w(z) \approx -0.95$ at $z \sim 1$),
- subtle modifications to black-hole ringdown quasi-normal modes.

These predictions arise collectively from the same deformation and are designed for near-term falsification with current or upcoming facilities (Planck/LiteBIRD/CMB-S4, LISA/BBO, CTA, Euclid/Roman, LIGO O5). Null results at conservative thresholds across these channels would rule out the framework; detections would support compression-driven emergence of gravity and the dark sectors from quantum geometry. Detailed falsification criteria and analysis protocols are provided in Sec.~\ref{sec:obs_tests}.

This approach stands out for its conceptual economy: a single microscopic mechanism unifies gravity's quantum origin with dark matter and dark energy phenomenology while preserving ultraviolet finiteness via discrete spinfoams and diffeomorphism invariance at leading order. The remainder of the paper is organized as follows: 
...
\begin{itemize}
    ...
    \item Sec.~\ref{sec:predictions} lists predicted observations
    ...
    \item Detailed derivations listed in Appendix~\ref{app:derivations}
    \item Dark sector implications discussed in Appendix~\ref{app:dm_de}
    \item Rigorous proofs appear in Appendix~\ref{app:proofs}
    \item Current actual observations shown in Appendix~\ref{app:current_obs}.
\end{itemize}
...
\FloatBarrier
\subsubsection{Units: Natural-Unit Reference}
...
\end{equation}
\label{eq:holo_entropy_constrain}
...
\textbf{Step 4: Emergence of Forces.}  
Compression (\(\uparrow \rho\)) damps high-spin modes (Eq.~\ref{eq:face_amplitude}), biasing toward low-curvature geometry. Entropy gradients then produce entropic forces (Appendix~\ref{app:entropic_force_derivation}).\cite{oriti2023}).
...
\begin{table}[ht]
\centering
\caption{Falsification thresholds for EQG's core predictions. A null result meeting any of these criteria at the stated significance would exclude the model or its key parameters.}
...
\midrule
CMB low-multipole suppression & Planck / LiteBIRD / CMB-S4 ($\ell \lesssim 20$) & $|\Delta C_\ell|/C_\ell < 5\%$ at 95\% CL bounds $\sigma \lesssim 0.01$ \\
Primordial GW red tilt & LISA / BBO (millihertz band) & No detection of $\Delta n_T \neq 0$ at $>4\sigma$ bounds $\eta \rho(t_k) < 10^{-2}$ \\
Isotropic gamma-ray lines (10--50 GeV) & CTA (GC + stacked dSph, 500--1000 hr) & Non-detection at $>99\%$ CL rules out $\eta > 0.2$--0.3 \\
Galaxy rotation curve corrections & SPARC / JWST / Euclid ($r > 50$ kpc) & No extra attraction beyond baryonic at $>4\sigma$ bounds $\alpha_\Phi < 10^3$ m$^2$ s$^{-2}$ \\
...
\begin{equation}
\addequation{Face Amplitude Deformation}{eq:appendix_face_amplitude}
...
This appendix details how dark matter and dark energy emerge from the single spinfoam deformation (Eq.~\ref{eq:face_amplitude}) modulated by the compression density \(\rho(t,x)\). Both effects arise from the same mechanism: holographic entropy gradients on emergent screens (Sec.~\ref{sec:entropic_chain}). Local compression excesses produce additional attraction (DM-like), while global dilution weakens it (DE-like).

\subsection{Dark Energy from Global Dilution}
\label{subsec:de_from_dilution}
The entropic force derives from the modified holographic bit count \(N \to N (1 - \eta \rho(t))\) (Eq.~\ref{eq:compressed_bits}), where \(\rho(t)\) is the cosmic compression density. In the low-\(\rho\) limit, the unmodified case recovers Newtonian gravity. Global dilution of \(\rho(t)\) (from LQC-motivated scaling, Appendix~\ref{app:density_derivations}) reduces the compression effect, increasing the number of effective independent bits over time and producing an effective repulsive term in the potential (Sec.~\ref{sec:potential_force}).
...
This produces evolving \(w(z) \neq -1\), testable with DESI/Euclid/Roman (see also Fig.~\ref{fig:wz_plot} and Fig.~\ref{fig:desi_wz_contour}).

\subsection{Dark Matter from Localized Entropy Excesses and SU(3) Glueballs}
Localized compressions \(\delta \rho(t,x)\) create entropy excesses on screens: \(\delta S_{\rm DM} \propto \exp(-r/r_{\rm DM})\). The gradient drives additional attraction (Yukawa term in potential, Eq.~\ref{eq:potential_per_mass}).
...
\begin{equation}
\addequation{Glueball Mass}{eq:glueball_mass_a22}
m_{\text{glue}}^2 = \frac{\eta \rho(t)}{\Lambda_\star} \quad (c = \hbar = 1),
\end{equation}
\label{eq:glueball_mass_a22}
...
This mechanism unifies DM halo profiles (Yukawa fit) and isotropic gamma signals without exotic particles beyond geometry.
...
\end{equation}
\label{eq:gamma_flux}
where \(\langle \sigma v \rangle \sim 10^{-26}\) cm³ s⁻¹ (thermal relic scale, phenomenological), \(\rho_{\rm DM}\) from localized excesses, and \(f_{\rm cont}\) a power-law continuum (e.g., \((E/m)^{-1.5}\) in toy sims). This yields isotropic lines detectable by CTA (Sec.~\ref{sec:predictions}).
...
\begin{itemize}
  ...
  \item 3. Entropy gradients from modified bits (Eq.~\ref{eq:modified_bits}) yield force (Eq.~\ref{eq:emergent_force}).
  \item 4. Local \(\delta\rho > 0\) → DM-like clustering (Sec.~\ref{subsec:pure_entropic_dm}).
  \item 5. Global dilution → DE-like repulsion (Sec.~\ref{subsec:de_from_dilution}).
\end{itemize}
...
\begin{itemize}
    ...
    \item 4. Gravity/DM/DE: Entropy gradients from modified bits (Eq.~\ref{eq:modified_bits}) yield force (Eq.~\ref{eq:emergent_grav_force}); local \(\rho\) ↑ → DM clustering; global dilution → DE (Theorem \ref{proof:emergence_from_compression}).
...
\begin{itemize}
  ...
  \item 3. With \(\eta\) coupling bit enhancement (Eq.~\ref{eq:modified_bits}), effective suppression \(\ell_p^3 \eta \rho C_j > 1\).
