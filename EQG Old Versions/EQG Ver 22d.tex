...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\textit{Emergent Quantum Gravity -- \DTMnow\ -- Version 22d}}
...
\end{enumerate}
These may align with exploratory concepts like ER=EPR, relating entanglement to geometry.
\end{abstract}
...
This functional form is not derived from first principles but is strongly motivated by two independent lines of reasoning (detailed in Appendix~\ref{app:density_derivations}):
\begin{enumerate}
...
The spinfoam formalism provides the microscopic path integral over quantum geometries. The standard partition function in Loop Quantum Gravity (LQG) is
\begin{equation}
...
\end{equation}
where \(\Gamma\) is a 4D simplicial complex, \(j_f\) are half-integer spins on faces (quantized area), \(i_e\) are intertwiners on edges (quantized volume), and \(A_f, A_e, A_v\) are amplitudes from the EPRL model.

EQG introduces a deformation to the face amplitudes motivated by compression effects:
\begin{equation}
\addequation{EQG Face Amplitude Deformation}{eq:face_amplitudes}
A_f(j_f) \to A_f(j_f) \, e^{-\ell_p^3 \rho(t,x) \, C_j}, \qquad C_j = j_f(j_f+1).
\end{equation}

This exponential suppresses high-spin modes in regions of high density \(\rho\), with dimensions consistent (\(\ell_p^3 \rho\) dimensionless). The deformed partition function becomes
\begin{equation}
...
In the cosmological limit, \(\rho(t,x) \to \rho(t) = \rho_0 / \sinh^3(\sqrt{\Lambda/3} \, t)\), motivated by LQC scaling (see \ref{app:density_derivations}).

This deformation is the core input, leading to emergent effects in the semiclassical limit.
...
S(r,t) = \frac{\pi r^2 c^3}{G \hbar} (1 - \eta \ell_p^3 \rho(t)) + \alpha \exp(-r/r_{\rm DM}) - \beta r^2 + \gamma \rho(t) + \xi(t) + \frac{c}{6} \log(r/\ell_p).
\end{equation}

Derivatives for the entropy gradient:
\begin{equation}
\addequation{Entropy Gradient}{eq:entropy_gradient}
\frac{dS}{dr} = \frac{2\pi r c^3}{G \hbar} (1 - \eta \ell_p^3 \rho(t)) - \frac{\alpha}{r_{\rm DM}} \exp(-r/r_{\rm DM}) - 2\beta r + \frac{c}{6r}.
\end{equation}

All terms are dimensionless; constants (\(\alpha, \beta, \gamma\)) chosen phenomenologically for empirical fit (see Sec.~\ref{sec:entropy_budget}).

\subsection{Potential and Force}
\label{app:potential_derivations}

The potential per unit mass follows from integrating \(F = T \, dS/dr\):
\begin{equation}
\addequation{Expanded EQG Potential}{eq:expanded_potential}
\Phi(r,t) = -\frac{G M}{r} (1 + \eta \ell_p^3 \rho(t)) - \frac{\Lambda(t) c^2}{6} r^2 + \alpha_\Phi \exp(-r/r_{\rm DM}) + \kappa_\Phi \log(r/\ell_p).
\end{equation}

Derivation term-by-term: See \ref{app:entropic_force_derivation}. Dimensions consistent (SI units).

\subsection{Units and Natural-Unit Reference}
\label{app:natural_units}

All equations use SI units unless noted. Natural units (\(c = \hbar = k_B = 1\)) are used for particle physics expressions, with conversions: $1\ \mathrm{GeV} = 1.602 \times 10^{-10}\ \mathrm{J}$.

\subsection{Density Derivation}
\label{app:density_derivations}

From LQC bounce solution: \(a(t) \propto \sinh^{2/3}(\sqrt{\Lambda/3} \, t)\), volume \(V(t) \propto \sinh^2(\sqrt{\Lambda/3} \, t)\). Quanta conservation implies \(\rho(t) \propto 1/\sinh^2\).

Phenomenological adjustment for late-time fit: \(\sinh^{-3}\):
\begin{equation}
\addequation{Phenomenological Compression Density}{eq:phenom_density}
\rho(t) = \rho_0 / \sinh^3(\sqrt{\Lambda/3} \, t).
\end{equation}

Dimensions: \([\rho] = \ell_p^{-3}\).

\subsection{Cosmic Microwave Background (CMB) Multipole Derivations}
\label{app:cmb_derivations}

Scalar perturbations from \(\xi(t)\): \(\Phi(k) \propto \xi(t_k)\). Transfer function yields power suppression at low \(\ell\). Bispectrum estimate:
\begin{equation}
\addequation{Bispectrum Estimate for Non-Gaussianity}{eq:bispectrum_fnl}
f_{\rm NL} \approx \sigma \rho(t_k).
\end{equation}

Falsification via \(\chi^2\) on Planck low-\(\ell\) likelihood.
...
Emergent metric: \(ds^2 = -dt^2 + a(t)^2 d\Omega^2\), \(a(t) \propto e^{H t}\). EQG deformation: \(e^{-S_{\text{Regge}} (1 + \eta \rho(t))}\).
...
Mass: \(m_{\text{glue}} = \sqrt{\eta \rho(t) / \Lambda_\star}\). Rate: \(\langle \sigma v \rangle \propto \alpha_{\text{GFT}}^2 / m_{\text{glue}}^2\).
...
Bridges Planck to TeV scales.

\subsection{Black-Hole Entropy Microstates}
\label{app:bh_entropy}

Compressed vertices yield:
\begin{equation}
\addequation{Modified Black Hole Entropy}{eq:spinfoam_compression}
S_{BH} = \frac{A}{4\ell_p^2} (1 - \eta \ell_p^3 \rho) = \ln \Omega,
\end{equation}

where \(\Omega\) counts deformed configurations.
...
S[\phi] = \int \bar\phi ( -\square_G + m^2_0 + \kappa \rho(t) ) \phi + \frac{\lambda}{2} (\bar\phi \phi)^2 + \mathcal{O}(\phi^6).
\end{equation}

Condensate: \(\phi = \phi_0 + \delta\phi\). Fluctuation operator:
\begin{equation}
\addequation{GFT Fluctuation Operator}{eq:fluct_operator}
...
\mathcal{H} = \int \mathrm{d}g_1\cdots\mathrm{d}g_4 \Bigl[ \phi (-\square_G + m_0^2 + \kappa \rho(t)) \phi + \frac{\lambda}{2} (\phi^2)^2 + \cdots \Bigr].
\end{equation}
...
H = \sum_{j_f i_e} \Bigl[ C_{j_f} + m_0^2 + \kappa \rho(t) \Bigr] |\phi_{j_f i_e}|^2 + \lambda \sum V(\{j_f,i_e\}) + \cdots.
\end{equation}

\subsection{Second Quantization of the GFT Hamiltonian and the \(\rho(t)\)-Deformed Spinfoam Measure}
...
\hat H = \sum_{j_f i_e} \omega_{j_f}(\rho(t))\, \hat a^\dagger_{j_f i_e} \hat a_{j_f i_e} + \frac{\lambda}{2} \sum \hat V(\{\hat a^\dagger, \hat a\}) + \cdots,
\end{equation}
...
\exp\!\bigl(- \omega_{j_f}(\rho(t)) \cdot \text{length}\bigr) \to \exp\!\bigl(-\kappa \rho(t) j_f(j_f+1)\bigr).
\end{equation}

\subsection{Renormalization and the Physical Meaning of \(\Lambda_\star\) and \(\eta\)}
...
Tilt deviation:
\begin{equation}
\addequation{Tensor Tilt Deviation Derivation}{eq:tensor_tilt_derivation}
...
Modified in EQG:
\begin{equation}
\addequation{Modified Entanglement Entropy in EQG}{eq:modified_ent_entropy}
S_{\rm ent}(r,t) = \frac{c}{6} \log\left(\frac{r}{\ell_p}\right) (1 - \eta \ell_p^3 \rho(t)) + S_0.
\end{equation}

\subsection{Linearized SVT Perturbations from Deformed Regge Calculus}
\label{app:tensor_perturbations}

Deformed Regge action:
\begin{equation}
...
\begin{equation}
\addequation{Compression-Modified Holographic Bits}{eq:modified_bits}
N = \frac{A c^3}{G \hbar} (1 - \eta \ell_p^3 \rho(t,x)).
\end{equation}

Equipartition:
\begin{equation}
\addequation{Equipartition Energy}{eq:equipartition_energy}
E = \frac{1}{2} N k_B T = M c^2.
\end{equation}

Acceleration:
\begin{equation}
\addequation{Acceleration from Equipartition}{eq:acceleration_from_equip}
a = \frac{4\pi M c^3}{N \hbar}.
\end{equation}

Substitute N:
\begin{equation}
\addequation{Acceleration with Modified Bits}{eq:acceleration_modified}
a = \frac{G M}{r^2} \frac{1}{1 - \eta \ell_p^3 \rho} \approx \frac{G M}{r^2} (1 + \eta \ell_p^3 \rho).
\end{equation}

Force:
\begin{equation}
\addequation{Emergent Gravitational Force from Compression}{eq:emergent_force_compression}
F = m a = \frac{G M m}{r^2} (1 + \eta \ell_p^3 \rho(t,x)).
\end{equation}
