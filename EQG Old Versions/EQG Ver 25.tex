...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\textit{Emergent Quantum Gravity -- \DTMnow\ -- Version 25}}
...
\begin{equation}
\addequation{Emergent Gravitational Force}{eq:entropic_force_derivation}
...
Isotropic gamma-ray lines (SU(3) extension) & CTA (GC + stacked dSph, 500--1000 hr) & Non-detection at $>99\%$ CL rules out $\eta > 0.2$--0.3 \\
...
\label{sec:su3_gft}
The hidden SU(3) sector models dark matter via glueballs, with non-Abelian action deformed by $\eta \rho(t)$ (Eq.~\ref{eq:glueball_mass}). Confinement at RG scale $\Lambda_\star$ yields stable scalars:
\begin{equation}
...
The pure-entropic mechanism offers a geometry-only explanation of dark matter clustering, keeping the model minimal and elegant. The SU(3) extension, while optional, provides a concrete and highly distinctive prediction (monochromatic gamma-ray lines) that can be pursued independently with CTA and Fermi data. If lines are detected, the model naturally pivots to the SU(3) branch; if not, the core framework remains intact.
...
\begin{equation}
\addequation{Predictive Alternative SU(3) Glueball Mass}{eq:su(3)_glueball_mass}
m_{\text{glue}}^2 = \frac{\eta \rho(t)}{\Lambda_\star} \quad (c = \hbar = 1),
\end{equation}
\label{eq:su(3)_glueball_mass}
...
\textbf{Proof}:
\begin{itemize}
  \item The deformation is scalar: $\exp(-\eta \ell_p^3 \rho C_j)$, with $\rho(t,x)$ transforming as a scalar density.
  \item The full measure remains invariant because the exponential commutes with the diffeomorphism constraint.
  \item In the effective action, the correction is scalar, preserving covariance.
\end{itemize}
...
\textbf{Proof}:
\begin{itemize}
  \item Compression invariance fixes $\rho = N/V$ (Theorem~\ref{theorem:compression_invariance}).
  \item Deformation damps high-j modes (Eq.~\ref{eq:face_amplitude}), biasing toward low-curvature geometry.
  \item Entropy gradients from modified bits (Eq.~\ref{eq:modified_bits}) yield force (Eq.~\ref{eq:emergent_force}).
  \item Local $\delta\rho > 0$ → DM-like clustering (Sec.~\ref{subsec:pure_entropic_dm}).
  \item Global dilution → DE-like repulsion (Sec.~\ref{subsec:de_from_dilution}).
\end{itemize}
...
\begin{itemize}
  ...
  \item 3. Entropy gradients from modified bits (Eq.~\ref{eq:modified_bits}) yield force (Eq.~\ref{eq:emergent_grav_force}).
...
In this scenario, gravity-mass compresses matter to Planck density (e.g., black holes). A Loop Quantum Cosmology bounce resets the spinfoam quanta \cite{ashtekar2017}, recompressing the network via energy-mass perturbations and increasing entropy (second law). This drives the three-branch emergence: gravity via entropic force (Eq.~\ref{eq:entropic_force_derivation}), DM via SU(3) glueballs (Eq.~\ref{eq:glueball_mass}), DE via dilution (Eq.~\ref{eq:entropic_force_derivation}), leading to macroscopic effects (curvature, halos, expansion).
