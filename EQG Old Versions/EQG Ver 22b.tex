...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\textit{Emergent Quantum Gravity -- \DTMnow\ -- Version 22b}}
...
\node[box, below=of micro] (deform) {Single Deformation\\$A_f \to A_f \exp(-\ell_p^3 \rho \, C_j)$\\(Appendix A.10–A.12)};
\node[box, below=of deform] (comp) {Compression Density $\rho(t,x)$\\Local $\delta\rho \propto T_{00} \ell_p^3$\\Lemmas in Appendix C};
...
\node[branchbox, right=1.0cm of gravity] (dm) {Dark Matter\\SU(3) glueballs (10–50 GeV)\\Gamma lines (Sec. 9.4)};
\node[branchbox, right=1.0cm of dm] (de) {Dark Energy\\Global dilution $\Lambda(t) \propto \rho(t)$\\Evolving $w(z)$ (Sec. 2.4)};
...
While EQG currently treats Standard Model fields as external inputs coupling via \(T_{\mu\nu}\), the GFT framework naturally allows for matter degrees of freedom to emerge as relational excitations entangled with geometry (Oriti 2023). Compression damping could bias toward stable particle-like states, potentially extending to fermions and gauge fields in future multi-group extensions. This remains speculative and beyond the current scope; however, the same mechanism that unifies gravity and dark sectors may offer a pathway to full matter emergence, warranting further investigation.

\subsection{Rationale for the SU(3) Extension}
The SU(3) hidden sector in EQG is a minimal, well-motivated addition grounded in tensorial Group Field Theory, where multi-group extensions naturally accommodate matter and dark sectors without direct coupling to the visible Standard Model beyond gravity \cite{geloun2013, geloun2016, oriti2014}. It avoids unnecessary complexity by mirroring QCD's non-Abelian structure, ensuring asymptotic freedom and confinement for stable glueballs as dark matter candidates. This choice is not phenomenological overreach but a logical step: smaller groups like SU(2) lack sufficient bound-state stability for the predicted 10--50 GeV mass range, while larger groups introduce redundant parameters (see lemma in Appendix~\ref{proof:su3_minimality_lemma} for proof of minimality). Literature support from recent GFT models confirms such extensions yield testable signatures like gamma lines, aligning with EQG's emphasis on falsifiability.
...
The parameters of EQG are summarized in Table~\ref{tab:parameters}. The emergence of gravity, dark matter, and dark energy is required by the compression mechanism when \(\eta > 0\) (Theorem \ref{proof:emergence_from_compression} in Appendix~\ref{app:proofs}). All are either fixed by renormalization group flow or constrained by near-term observations, yielding six independent falsifiable predictions (Sec.~\ref{sec:predictions}).
...
EQG demonstrates that a single, GFT-derived deformation of spinfoam amplitudes can account for gravity and the dark sectors in strict 4D, offering a testable path toward quantum gravity. "The emergence is rigorously required by the compression mechanism (Theorem in Appendix \ref{proof:emergence_from_compression})
