...
The model depends on a small set of parameters whose status is summarized in Table 2 below.
...
\subsubsection{Derivation of GFT Condensate Emergence}
\label{app:gft_condensate_derivation}

Group Field Theory (GFT) provides the second-quantized foundation for LQG, where spin networks are "particles" and spin foams Feynman diagrams \cite{oriti2014}. The condensate phase emerges from mean-field dynamics of the GFT field \(\phi\), yielding effective cosmology \cite{oriti2016, gielen2016}.

The GFT action for a scalar field on $\mathrm{SU}(2)^4$ is:
\begin{equation}
\addequation{GFT Action for Condensate}{eq:gft_action_condensate}
S[\phi] = \int \bar\phi ( -\square_G + m^2 ) \phi + \lambda \int \phi^4 + \mathcal{O}(\phi^6),
\end{equation}
\label{eq:gft_action_condesate}
with \(\square_G\) the group Laplacian. In the condensate phase, \(\phi = \phi_0 + \delta\phi\), where \(\phi_0\) is constant (homogeneous). The fluctuation equation is Klein-Gordon-like:
\begin{equation}
\addequation{Fluctuation Equation in Condensate}{eq:fluct_eq_condensate}
( \square_G + m^2 + 3\lambda |\phi_0|^2 ) \delta\phi = 0.
\end{equation}
The condensate density \(\rho(t) = |\phi_0|^2\) shifts the effective mass, damping high-momentum modes. In spin basis, the propagator becomes \(G(j) \propto 1/(j(j+1) + m^2 + \rho(t))\), leading to the deformation exp($-\rho C_j$) for face amplitudes (see App.~\ref{app:spinfoam_deformation}).

This condensate emerges naturally in GFT as a low-spin phase, where most quanta have j~0 or 1/2, yielding smooth geometry \cite{gielen2016}. In EQG, energy-mass back-reaction (via $T_{00}$) increases \(\rho(t)\), biasing to classical emergence \cite{oriti2021tensorial}. This supports EQG's unification: condensate provides the "sea" of quanta from which spacetime/DM/DE arise.
...
The author was aided by xAI Grok and OpenAI ChapGPT for literature research, ensuring mathematical and data integrity and some organizational arrangement.
