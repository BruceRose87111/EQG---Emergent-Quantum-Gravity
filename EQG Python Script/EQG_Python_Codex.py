...
OUTPUT_DIR = r'C:\Users\brose\OneDrive\Desktop\GROK E-Q-G\EQG Images\3D'
...
def wz_plot():
    """
    Generate w(z) comparison plot with approximate-but-stable w(z) from sinh^3 scaling.
    Uses cumulative_trapezoid (SciPy >=1.12 replacement for cumtrapz).
    Saves as 'wz_plot.png' in OUTPUT_DIR.
    """
    # Constants (physical units)
    G = 6.67430e-11          # m³ kg⁻¹ s⁻²
    c = 2.99792458e8         # m/s
    H0_km_s_Mpc = 70.0       # Hubble constant today
    H0 = H0_km_s_Mpc / 3.08568e19  # s⁻¹
    Omega_m0 = 0.3           # Matter density today
    Omega_Lambda = 0.7       # Dark energy today (approx)
    Lambda = 3 * H0**2 * Omega_Lambda / c**2  # Cosmological constant (m⁻²)

    rho0 = 1.0               # Normalized compression density today

    # rho(t) from sinh^3 (normalized)
    def rho(t):
        return rho0 / np.sinh(np.sqrt(Lambda / 3) * t)**3

    # Approximate H(z) = H0 * sqrt(Omega_m (1+z)^3 + Omega_Lambda)
    # Good for z < 3; ignores radiation (negligible late-time)
    def H_z(z):
        return H0 * np.sqrt(Omega_m0 * (1 + z)**3 + Omega_Lambda)

    # t(z) = ∫_0^z dz' / ((1+z') H(z'))
    def t_from_z(z):
        def integrand(zp):
            return 1 / ((1 + zp) * H_z(zp))
        t, _ = quad(integrand, 0, z, limit=100)
        return t

    # w(z) = -1 + (1+z)/3 * d ln rho_DE / dz
    def w(z):
        dz = 1e-3
        t_plus = t_from_z(z + dz)
        t_minus = t_from_z(z - dz)
        ln_rho_plus = np.log(rho(t_plus))
        ln_rho_minus = np.log(rho(t_minus))
        dln_dz = (ln_rho_plus - ln_rho_minus) / (2 * dz)
        return -1 + (1 + z) / 3 * dln_dz

    # Redshift range
    z = np.linspace(0, 3, 300)

    # Compute w_eqg
    w_eqg = np.array([w(zi) for zi in z])

    # ΛCDM reference
    w_lcdm = np.full_like(z, -1.0)

    # DES Y6 hint band
    w_des_center = -0.95 + 0.01 * (z - 1.0)  # mild tilt
    w_des_lower = w_des_center - 0.05
    w_des_upper = w_des_center + 0.05

    fig, ax = plt.subplots(figsize=(10, 7), dpi=600)

    ax.plot(z, w_lcdm, color='black', linestyle='--', linewidth=2.5, label=r'$\Lambda$CDM (w = -1)')
    ax.plot(z, w_eqg, color='blue', linewidth=3, label=r'EQG (from $\rho(t) \propto 1/\sinh^3$)')
    ax.fill_between(z, w_des_lower, w_des_upper, color='orange', alpha=0.3, label=r'DES Y6 hints ($\pm$0.05)')
    ax.axhline(-1, color='gray', linestyle=':', linewidth=1.5, alpha=0.7)

    ax.set_xlabel('Redshift z', fontsize=14)
    ax.set_ylabel('Equation of State w(z)', fontsize=14)
    ax.set_title(r'EQG w(z) vs. $\Lambda$CDM and DES Y6 Hints', fontsize=16, pad=20)
    ax.set_ylim(-1.15, -0.80)
    ax.set_xlim(0, 3)
    ax.grid(True, alpha=0.4, linestyle='--')

    ax.legend(fontsize=12, loc='upper left', frameon=True, framealpha=0.9)

    ax.annotate(r'DES Y6 preference for dynamic DE' '\n' r'(w ≈ -0.95 at z≈1, 3–4σ tension with w=-1)',
                xy=(1.0, -0.95), xytext=(1.2, -1.10), fontsize=12,
                arrowprops=dict(facecolor='black', shrink=0.05, width=1.5, headwidth=8),
                bbox=dict(boxstyle="round,pad=0.3", fc="white", ec="gray", alpha=0.8))

    output_path = os.path.join(OUTPUT_DIR, 'wz_plot.png')
    plt.tight_layout()
    plt.savefig(output_path, dpi=600, bbox_inches='tight')
    plt.close()
    print(f"Generated: {output_path}")
